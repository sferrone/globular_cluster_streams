...
contains
  ...
  SUBROUTINE orbits(x,y,z,vx,vy,vz,GCparam,MWparam,model,modelshort,GCname,backward,pathorbits,deltat,Nstep,isave,N)
...
!! time in units of 0.1 Gyr, as in Allen & Santillan 1991,Pouliasis et al 2017
  ...
  INTEGER,intent(in):: N,Nstep,isave
  ...
  CALL INPUTDATA(backward,model,modelshort,GCname,Nstep,omega_bar,xGCv,yGCv,zGCv,&
       vxGCv,vyGCv,vzGCv)
  ...
  DO istep=1,Nstep
     ...
     IF(backward.EQ.'NO') THEN
        IF(mod(istep,isave).EQ.0) THEN
           it=int(istep/(isave*1.))
           write(lab,'(I5)')it ! convert integer to character                       
           SELECT CASE(it)
           CASE(1:9)
              lab ='00'//adjustl(lab)
           CASE(10:99)
           ...
           CASE(100:999)
              lab=adjustl(lab)
           END SELECT
           OPEN(UNIT=22,FILE='output'//TRIM(lab)//'.bin',FORM='UNFORMATTED')
           WRITE(22)SNGL(xp)
           WRITE(22)SNGL(yp)
           WRITE(22)SNGL(zp)
           WRITE(22)SNGL(vxp)
...
SUBROUTINE INPUTDATA(backward,model,modelshort,GCname,Nstep,omega_bar,xGCv,yGCv,zGCv,&
     vxGCv,vyGCv,vzGCv)

  IMPLICIT NONE

  INTEGER,INTENT(IN) :: Nstep
  CHARACTER*100,INTENT(IN) :: model,modelshort,backward,GCname
  ...
  IF(backward.EQ.'NO') THEN
     OPEN(UNIT=50,FILE='../outputDATA/'//TRIM(model)//'/backwardorbits'//'/orbit'//TRIM(GCname)//'.dat',FORM='FORMATTED')
     DO istep=1,Nstep+1
