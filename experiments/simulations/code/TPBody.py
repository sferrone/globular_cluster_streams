...
"""
ASSIGN GLOBULAR CLUSTER POTENTIAL
"""
gc_model="Plummer"
...
for number in range(0,n_trials):
    ...
    pathorbits = "/"+gc_model+'/backwardorbits'
    output.createdir('../outputDATA/'+model)
    output.createdir('../outputDATA/'+model+"/"+gc_model+"/")
    ...
    integrate.orbits(xGC,yGC,zGC,vxGC,vyGC,vzGC,GCparam,MWparam,model,modelshort,GCname,backward,pathorbits,deltat,Nstep,N)
...
#now integrate the N-body GC forward in time
    ...
    if gc_model=="Plummer":
        ...
        error("Globular cluster potential not found")
    MWparam=inputMW.MW(modelshort)
    
    integrate.deallocate()
    integrate.orbits(x0,y0,z0,vx0,vy0,vz0,GCparam,MWparam,model,modelshort,GCname,backward,pathorbits,deltat,Nstep,N)
    ...
    if(ierror==1):
        ...
        hf = h5py.File('../outputDATA/'+model+pathorbits+GCname+"_"+str(number)+'.h5', 'w')
