...
class Streams:
    ...
    def set_axis_properties(self,
        ...
        if self.coordainte_sys == 'xy':
            self._ax.set_xlim(x_min, x_max)
            self._ax.set_ylim(y_min ,y_max)
            self._ax.set_xlabel("x (kpc)", size=font_size)
            self._ax.set_ylabel("y (kpc)", size=font_size)
        elif self.coordainte_sys == 'rz':
            self._ax.set_xlim(0, x_max)
            self._ax.set_ylim(y_min ,y_max)
            self._ax.set_xlabel("r (kpc)", size=font_size)
            self._ax.set_ylabel("Z (kpc)", size=font_size)
...
def main():
    ...
    my_stream           = Streams(GCName, coordainte_sys=units)
    # establish the proper number of iterations
    start_time = 1
    end_time = 500 # in 10s of millions of years 
    interval = 1
