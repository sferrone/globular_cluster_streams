...
for fname in fnames:
    ...
    my_latitude.append(np.array(data_file['galactic']['LAT']))
    my_longitude.append(np.array(data_file['galactic']['LONG']))
longitude = np.hstack(my_longitude)    
latitude = np.hstack(my_latitude)    
...
ax.grid(True)
# plot the data 
im = ax.scatter(longitude, latitude, c=quantity, s=dot_size)
...
ax.set_ylim(-90,90)
ax.set_aspect('equal')
ax.set_xlabel("Galactic Longitude", size=my_fontsize)
ax.set_ylabel("Galactic Latitude",size=my_fontsize)
...
n_files = len(fnames)
outname = "galactic_latitude_longitude_quantity_"+quantity_name+"_nClusters_"+str(n_files)+".png"
